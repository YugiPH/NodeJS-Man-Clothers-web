...
let listCustomerSide = async (req, res, next) => {
    try {
        ...
        let whereClause = category_id && Number.isInteger(category_id) ? { category_id } : {};
        ...
        listProductVariant.forEach(item => {
            let key = `${item.Product.product_id}-${item.colour_id}`;
            if (!groupedProducts[key]) {
                groupedProducts[key] = {
                    ...
                    feedback_quantity: item.Product.feedback_quantity,
                    product_variant_id: item.product_variant_id,
                    colour_id: item.colour_id,
                    colour_name: item.Colour?.colour_name || "N/A",
                    price: item.Product.Product_Price_Histories?.[0]?.price || 0,
                    product_image: item.Product_Images ? item.Product_Images[0].path : null,
                    sizes: new Set()
                };
            }
            if (item.Size?.size_name) {
                groupedProducts[key].sizes.add(item.Size.size_name);
            }
        });

        let finalList = Object.values(groupedProducts).map(product => ({
            ...product,
            sizes: [...product.sizes]
        }));
